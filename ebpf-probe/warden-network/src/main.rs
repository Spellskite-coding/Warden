...
#[derive(Parser)]
#[command(name = "warden-network", about = "Outbound-connection detection for Warden, via eBPF")]
struct Args {
...
/// Mirrors `warden-network-ebpf`'s `ConnectEvent { pid: i32, dport: u16,
/// family: u16, daddr: [u8; 16] }` (repr(C), no padding: 4 + 2 + 2 + 16 =
/// 24 bytes) - parsed back out of raw bytes here rather than shared as a
/// common type, since the ebpf crate can't depend on anything requiring
/// std.
fn parse_event(bytes: &[u8]) -> Option<ConnectEvent> {
    ...
    Some(ConnectEvent {
        ...
        dport: u16::from_ne_bytes(bytes[4..6].try_into().ok()?),
        family: u16::from_ne_bytes(bytes[6..8].try_into().ok()?),
        daddr: bytes[8..24].try_into().ok()?,
    })
}
...
async fn handle_connect(event: &ConnectEvent, exe_path: &str, mode: Mode, quarantine: &Quarantine, notifier: &Notifier) {
    let daddr = format_daddr(event);
    let reason = format!("process at {exe_path} (a suspicious location) opened an outbound connection to {daddr}:{}", event.dport);
    // Same reasoning as the exec module: don't skip a vanished binary, the
    // live process making the connection is what matters, and
    // response::handle_detection still kills it by pid either way.
...
async fn main() -> Result<()> {
    ...
    info!("network module ready, watching outbound connection attempts");
        ...
        tokio::select! {
            guard_res = poll.readable_mut() => {
                ...
                while let Some(item) = ring_buf.next() {
                    ...
                    tracing::debug!(pid = event.pid, dport = event.dport, daddr = %format_daddr(&event), "connection attempt observed");
                    if event.pid == own_pid {
                        continue;
                    }
                    // The tracepoint has no visibility into which binary
                    // the connecting process runs - resolved here from
                    // /proc, which can race a very short-lived process
                    // (already exited by the time we look it up); a
                    // vanished /proc/<pid>/exe just means nothing to flag,
                    ...
                    if is_suspicious_exec_location(&exe_path, &target) {
                        handle_connect(&event, &exe_path, cfg.mode, &quarantine, &notifier).await;
                    }
                }
                guard.clear_ready();
            }
...
mod tests {
    ...
    #[test]
    fn parses_a_well_formed_ipv4_event() {
        ...
        bytes[8..12].copy_from_slice(&[93, 184, 216, 34]);
    ...
    fn formats_ipv6_addresses() {
        ...
        daddr[15] = 1;
        let event = ConnectEvent { pid: 1, dport: 80, family: AF_INET6, daddr };
